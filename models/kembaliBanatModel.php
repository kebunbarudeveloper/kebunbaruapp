...
class kembaliBanatModel extends CI_Model
{
    public function getDomicile()
    {
        return $this->db->distinct()->select('domisili_santri')->from('data_santri')->where('domisili_santri !=', '')->get()->result_object();
    }

    public function gettanggal()
    {
        $data = $this->db->get('set_date_banat')->row_object();
        if ($data) {
            $hasil = $data->tanggal;
        } else {
            $hasil = 0;
    ...
    public function tanggal()
    {
        ...
        $this->db->empty_table('set_date_banat');

        $this->db->insert('set_date_banat', [
            'tanggal' => $jadi,
        ]);
    ...
    public function setkembalian()
    {
        ...
        $cek = $this->db->get_where('kembalian_banat', [
            'periode' => $periode
        ])->num_rows();

        if ($cek > 0) {
            $getdata = $this->db->get_where('kembalian_banat', [
                'periode' => $periode
            ])->result_object();
            ...
            $this->db->update_batch('kembalian_banat', $datax, 'id');
        } else {
            ...
            $this->db->insert_batch('kembalian_banat', $datax);
        }
    }
    ...
    public function getdata()
    {
        ...
        $data = $this->db->select('COUNT(santri_id) AS total')->from('kembalian_banat')->where([
            'periode' => $periode,
            'status <=' => 1,
        ])->get()->row_object();

        //Seluruh santri sudah kembali
        $data1 = $this->db->select('COUNT(santri_id) AS total')->from('kembalian_banat')->where([
            'periode' => $periode,
            'status >=' => 2,
            'status_kembali' => 1,
        ])->get()->row_object();
        ...
        //Kembali disiplin
        $data4 = $this->db->select('COUNT(santri_id) AS total')->from('kembalian_banat')->where([
            'periode' => $periode,
            'status' => 3,
        ])->get()->row_object();

        //Kembali telat
        $data5 = $this->db->select('COUNT(santri_id) AS total')->from('kembalian_banat')->where([
            'periode' => $periode,
            'status' => 2,
        ])->get()->row_object();

        //Seluruh santri belum kembali tanpa ijin
        $data10 = $this->db->select('COUNT(santri_id) AS total')->from('kembalian_banat')->where([
            'periode' => $periode,
            'status' => 0,
        ])->get()->row_object();

        //Seluruh santri belum kembali ijin
        $data11 = $this->db->select('COUNT(santri_id) AS total')->from('kembalian_banat')->where([
            'periode' => $periode,
            'status' => 1,
        ])->get()->row_object();
    ...
    public function showfilter()
    {
        ...
        $this->db->select('*')->from('kembalian_banat');
    ...
    public function save()
    {
        ...
        if ($id != '') {
            ...
            if ($cekID == 0) {
                ...
                $this->db->where(['santri_id' => $id, 'periode' => $periode])->update('kembalian_banat', $data);
                $hasil = 0;
            }
        } else {
            //If id is empty
    ...
    {
        return $this->db->get_where('kembalian_banat', [
            'santri_id' => $id,
            'periode' => $periode,
            'status >' => 0
        ])->num_rows();
    ...
    {
        $id = $this->input->post('id', true);
        $this->db->where('id', $id)->update('kembalian_banat', ['kembali' => NULL, 'status' => 0, 'alasan' => NULL]);
    }
